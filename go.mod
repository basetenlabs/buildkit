...
require (
	...
	github.com/awslabs/soci-snapshotter v0.0.0-20260609012424-f268898c03a8
...
// soci-snapshotter requires a newer containerd (v2.2.x) than buildkit builds
// against; pin it back to buildkit's containerd v2.0.5.
replace github.com/containerd/containerd/v2 => github.com/containerd/containerd/v2 v2.0.5

// soci-snapshotter also pulls the opencontainers spec set that goes with containerd
// v2.2.x: runtime-spec v1.3.0 plus the runtime-tools and container-device-interface
// versions built against it. containerd v2.0.5's pkg/oci was compiled against
// runtime-spec v1.2.0 (LinuxPids.Limit is int64 not *int64; no Linux.NetDevices or
// LinuxIntelRdt.Schemata), so the v1.3.0 set does not compile against it. Pin the
// whole set back to the v1.2.0-compatible versions to keep the build coherent.
//
// These pins can be dropped once buildkit moves to a containerd release that uses
// runtime-spec v1.3.0 (e.g. v2.2.x), which realigns the set with soci-snapshotter.
replace github.com/opencontainers/runtime-spec => github.com/opencontainers/runtime-spec v1.2.0

replace github.com/opencontainers/runtime-tools => github.com/opencontainers/runtime-tools v0.9.1-0.20221107090550-2e043c6bd626

replace tags.cncf.io/container-device-interface => tags.cncf.io/container-device-interface v1.0.1

replace tags.cncf.io/container-device-interface/specs-go => tags.cncf.io/container-device-interface/specs-go v1.0.0
